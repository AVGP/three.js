...
THREE.CurvePath.prototype.getWrapPoints=function(a,b){var c=this.getBoundingBox(),d,f,e,g,h,k;d=0;for(f=a.length;d<f;d++)e=a[d],g=e.x,h=e.y,k=g/c.maxX,k=b.getUtoTmapping(k,g),g=b.getPoint(k),k=b.getTangent(k),k.set(-k.y,k.x).multiplyScalar(h),e.x=g.x+k.x,e.y=g.y+k.y;return a};THREE.Gyroscope=function(){THREE.Object3D.call(this)};THREE.Gyroscope.prototype=Object.create(THREE.Object3D.prototype);THREE.Gyroscope.prototype.constructor=THREE.Gyroscope;
...
hole:l}),r?(r=!1,h[t].push(n)):s=!0);r&&h[g].push(n)}0<p.length&&(s||(q=h))}s=0;for(p=m.length;s<p;s++)for(h=m[s].s,k.push(h),g=q[s],f=0,e=g.length;f<e;f++)h.holes.push(g[f].h);return k};THREE.Shape=function(){THREE.Path.apply(this,arguments);this.holes=[]};THREE.Shape.prototype=Object.create(THREE.Path.prototype);THREE.Shape.prototype.constructor=THREE.Shape;THREE.Shape.prototype.extrude=function(a){return new THREE.ExtrudeGeometry(this,a)};
...
c.matrix);for(var c=c.animationCache.animations[this.data.name],d=0;3>d;d++){for(var f=this.keyTypes[d],e=this.data.hierarchy[a].keys[0],g=this.getNextKeyWith(f,a,1);g.time<this.currentTime&&g.index>e.index;)e=g,g=this.getNextKeyWith(f,a,g.index+1);c.prevKey[f]=e;c.nextKey[f]=g}}};
...
(-3*(b-c)-2*a-d)*f+a*e+b};return function(e){if(!1!==this.isPlaying&&(this.currentTime+=e*this.timeScale,0!==this.weight)){e=this.data.length;if(this.currentTime>e||0>this.currentTime)this.loop?(this.currentTime%=e,0>this.currentTime&&(this.currentTime+=e),this.reset()):this.stop();e=0;for(var h=this.hierarchy.length;e<h;e++)for(var k=this.hierarchy[e],l=k.animationCache.animations[this.data.name],m=k.animationCache.blending,q=0;3>q;q++){var n=this.keyTypes[q],r=l.prevKey[n],s=l.nextKey[n];if(0<this.timeScale&&
s.time<=this.currentTime||0>this.timeScale&&r.time>=this.currentTime){r=this.data.hierarchy[e].keys[0];for(s=this.getNextKeyWith(n,e,1);s.time<this.currentTime&&s.index>r.index;)r=s,s=this.getNextKeyWith(n,e,s.index+1);l.prevKey[n]=r;l.nextKey[n]=s}var p=(this.currentTime-r.time)/(s.time-r.time),t=r[n],u=s[n];0>p&&(p=0);1<p&&(p=1);if("pos"===n)if(this.interpolationType===THREE.AnimationHandler.LINEAR)c.x=t[0]+(u[0]-t[0])*p,c.y=t[1]+(u[1]-t[1])*p,c.z=t[2]+(u[2]-t[2])*p,r=this.weight/(this.weight+m.positionWeight),
k.position.lerp(c,r),m.positionWeight+=this.weight;else{if(this.interpolationType===THREE.AnimationHandler.CATMULLROM||this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD)a[0]=this.getPrevKeyWith("pos",e,r.index-1).pos,a[1]=t,a[2]=u,a[3]=this.getNextKeyWith("pos",e,s.index+1).pos,p=.33*p+.33,s=f(a,p),r=this.weight/(this.weight+m.positionWeight),m.positionWeight+=this.weight,n=k.position,n.x+=(s[0]-n.x)*r,n.y+=(s[1]-n.y)*r,n.z+=(s[2]-n.z)*r,this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD&&
(p=f(a,1.01*p),b.set(p[0],p[1],p[2]),b.sub(n),b.y=0,b.normalize(),p=Math.atan2(b.x,b.z),k.rotation.set(0,p,0))}else"rot"===n?(THREE.Quaternion.slerp(t,u,d,p),0===m.quaternionWeight?(k.quaternion.copy(d),m.quaternionWeight=this.weight):(r=this.weight/(this.weight+m.quaternionWeight),THREE.Quaternion.slerp(k.quaternion,d,k.quaternion,r),m.quaternionWeight+=this.weight)):"scl"===n&&(c.x=t[0]+(u[0]-t[0])*p,c.y=t[1]+(u[1]-t[1])*p,c.z=t[2]+(u[2]-t[2])*p,r=this.weight/(this.weight+m.scaleWeight),k.scale.lerp(c,
r),m.scaleWeight+=this.weight)}return!0}}}();THREE.Animation.prototype.getNextKeyWith=function(a,b,c){var d=this.data.hierarchy[b].keys;for(c=this.interpolationType===THREE.AnimationHandler.CATMULLROM||this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD?c<d.length-1?c:d.length-1:c%d.length;c<d.length;c++)if(void 0!==d[c][a])return d[c];return this.data.hierarchy[b].keys[0]};
...
0,1);for(f=1;f<=b;f++)this.faces.push(new THREE.Face3(f,f+1,0,[c.clone(),c.clone(),c.clone()])),this.faceVertexUvs[0].push([e[f].clone(),e[f+1].clone(),g.clone()]);this.computeFaceNormals();this.boundingSphere=new THREE.Sphere(new THREE.Vector3,a)};THREE.CircleGeometry.prototype=Object.create(THREE.Geometry.prototype);THREE.CircleGeometry.prototype.constructor=THREE.CircleGeometry;
THREE.CubeGeometry=function(a,b,c,d,f,e){console.warn("THREE.CubeGeometry has been renamed to THREE.BoxGeometry.");return new THREE.BoxGeometry(a,b,c,d,f,e)};
...
0;p<c;p++)e=p+g*(r+1),h=p+1+g*(r+1),k=p+1+g*r,f[q]=p+g*r,f[q+1]=e,f[q+2]=k,f[q+3]=e,f[q+4]=h,f[q+5]=k,q+=6;this.addAttribute("index",new THREE.BufferAttribute(f,1));this.addAttribute("position",new THREE.BufferAttribute(b,3));this.addAttribute("normal",new THREE.BufferAttribute(a,3));this.addAttribute("uv",new THREE.BufferAttribute(m,2))};THREE.PlaneBufferGeometry.prototype=Object.create(THREE.BufferGeometry.prototype);THREE.PlaneBufferGeometry.prototype.constructor=THREE.PlaneBufferGeometry;
...
THREE.TubeGeometry=function(a,b,c,d,f,e){THREE.Geometry.call(this);this.type="TubeGeometry";this.parameters={path:a,segments:b,radius:c,radialSegments:d,closed:f};b=b||64;c=c||1;d=d||8;f=f||!1;e=e||THREE.TubeGeometry.NoTaper;var g=[],h,k,l=b+1,m,q,n,r,s,p=new THREE.Vector3,t,u,w;t=new THREE.TubeGeometry.FrenetFrames(a,b,f);u=t.normals;w=t.binormals;this.tangents=t.tangents;this.normals=u;this.binormals=w;for(t=0;t<l;t++)for(g[t]=[],m=t/(l-1),s=a.getPointAt(m),h=u[t],k=w[t],n=c*e(m),m=0;m<d;m++)q=
m/d*2*Math.PI,r=-n*Math.cos(q),q=n*Math.sin(q),p.copy(s),p.x+=r*h.x+q*k.x,p.y+=r*h.y+q*k.y,p.z+=r*h.z+q*k.z,g[t][m]=this.vertices.push(new THREE.Vector3(p.x,p.y,p.z))-1;for(t=0;t<b;t++)for(m=0;m<d;m++)e=f?(t+1)%b:t+1,l=(m+1)%d,a=g[t][m],c=g[e][m],e=g[e][l],l=g[t][l],p=new THREE.Vector2(t/b,m/d),u=new THREE.Vector2((t+1)/b,m/d),w=new THREE.Vector2((t+1)/b,(m+1)/d),h=new THREE.Vector2(t/b,(m+1)/d),this.faces.push(new THREE.Face3(a,c,l)),this.faceVertexUvs[0].push([p,u,h]),this.faces.push(new THREE.Face3(c,
e,l)),this.faceVertexUvs[0].push([u.clone(),w,h.clone()]);this.computeFaceNormals();this.computeVertexNormals()};THREE.TubeGeometry.prototype=Object.create(THREE.Geometry.prototype);THREE.TubeGeometry.prototype.constructor=THREE.TubeGeometry;THREE.TubeGeometry.NoTaper=function(a){return 1};THREE.TubeGeometry.SinusoidalTaper=function(a){return Math.sin(Math.PI*a)};
...
Math.pow(2,b);Math.pow(4,b);for(var d=f(k.vertices[a.a]),g=f(k.vertices[a.b]),h=f(k.vertices[a.c]),l=[],m=0;m<=c;m++){l[m]=[];for(var n=f(d.clone().lerp(h,m/c)),p=f(g.clone().lerp(h,m/c)),q=c-m,r=0;r<=q;r++)l[m][r]=0==r&&m==c?n:f(n.clone().lerp(p,r/q))}for(m=0;m<c;m++)for(r=0;r<2*(c-m)-1;r++)d=Math.floor(r/2),0==r%2?e(l[m][d+1],l[m+1][d],l[m][d]):e(l[m][d+1],l[m+1][d+1],l[m+1][d])}function h(a,b,c){0>c&&1===a.x&&(a=new THREE.Vector2(a.x-1,a.y));0===b.x&&0===b.z&&(a=new THREE.Vector2(c/2/Math.PI+.5,
...
d,k)),e.push([q,n,s]),f.push(new THREE.Face3(d,l,k)),e.push([n.clone(),r,s.clone()]);this.computeFaceNormals();this.computeVertexNormals()};THREE.ParametricGeometry.prototype=Object.create(THREE.Geometry.prototype);THREE.ParametricGeometry.prototype.constructor=THREE.ParametricGeometry;
...
this.cone=new THREE.Mesh(b,new THREE.MeshBasicMaterial({color:e}));this.cone.matrixAutoUpdate=!1;this.add(this.cone);this.setDirection(c);this.setLength(f,g,h)}}();THREE.ArrowHelper.prototype=Object.create(THREE.Object3D.prototype);THREE.ArrowHelper.prototype.constructor=THREE.ArrowHelper;
